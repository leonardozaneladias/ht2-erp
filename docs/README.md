...
| [`superficie-do-core.md`](superficie-do-core.md)                  | O que uma extensão consome do core — medido na primeira extração; especificação do futuro `ht2ml/core`                               |
...
4. **Extrair um módulo para pacote** → [`superficie-do-core.md`](superficie-do-core.md) + [`distribuicao-manutencao.md`](distribuicao-manutencao.md)
5. **Exemplo de referência do stack** → módulo Usuários Admin em `app/Livewire/Admin/Usuarios/`, `app/Services/Admin/AdminUserService.php`, `app/Actions/Admin/*Action.php` (mostra FormRequest + Service + Action + DTO + Policy + Livewire + Activity Log + testes Pest aplicados juntos)
