...
- **Filtros de coluna (atualizado 2026-06-12):** filtros de seleção usam `Filter::multiSelect()` (TomSelect com **busca embutida** + **múltipla seleção**), não `Filter::select()` nativo, sempre que houver lista de opções. O TomSelect vem no bundle do PowerGrid; o CSS estrutural é importado via npm (`tom-select`) em `resources/css/admin.css` (`layer(components)`, antes da skin Inspinia `.ts-*`). Config em `config/livewire-powergrid.php` → `plugins.select.tom` (`dropdownParent: 'body'` para escapar do `overflow` da tabela). Filtros com relação (ex.: `perfis` via roles) usam `->builder()` com `whereIn` e guard de vazio. `Filter::boolean` (poucas opções) permanece select nativo.
