...
| 49a | `x-rh::event-timeline`                       | Table     | (doc 06 §6.1)                                               |    🟢    |     P2     |   🟢   | 🧩 Timeline VERTICAL de eventos PONTUAIS (data + tipo + antes→depois) da linha do tempo do RH. Mora no **pacote modulo-rh** (namespace `rh::`, ADR-0015), não no core. Distinto do item 49 (períodos). Recebe itens já projetados pelo Livewire; sem `<select>` nativo, sem CSS custom. |
| 49b | `x-rh::afastamento-item`                     | Table     | (doc 06 §5)                                                 |    🟢    |     P2     |   🟢   |                 🧩 Linha de um afastamento (tipo + período + badge "em curso/atrasado" + CID já mascarado) na aba Afastamentos do RH. Ações vêm pelo `slot`. Pacote **modulo-rh** (`rh::`, ADR-0015). Recebe o array projetado pelo Livewire GerenciadorAfastamentosRh.                 |
...
- Não existe `x-admin.timeline-item` no catálogo oficial. Para **períodos** (programações com início/fim/status) use `x-admin.timeline-table` (item 49); para **eventos pontuais** (histórico funcional do RH — data + tipo + antes→depois) há o `x-rh::event-timeline` (item 49a, no pacote modulo-rh).
