...
| Componentes já criados (status 🟢)                               | **58**  |
| Componentes 🔴 (não iniciados)                                   |  **8**  |
...
- **Norma de larguras/grade de formulário** (base 12, spans por tipo de campo, grade de leitura das fichas, números à direita em tabelas): [Forms/form-grid.md](template/INSPINIA/Forms/form-grid.md) — criada na D18 do doc 31 do RH.
...
| 22a | `x-shared.field-display` | UI             | [field-display.md](template/INSPINIA/Components/UI/field-display.md) |    🟢    |     P1     |   🟢   |      ✅ rótulo + valor read-only (tijolo das fichas "Ver"; `x-rh::field-display` delega aqui)      |
| 22b | `x-admin.ficha-drawer`   | UI (offcanvas) | [ficha-drawer.md](template/INSPINIA/Components/UI/ficha-drawer.md)   |    🟢    |     P1     |   🟢   | 🧩 composição sobre `x-admin.drawer` (`wide`+`blur`) p/ a ficha "Ver" dos CRUDs (trait `ComFicha`) |
| 22c | `x-shared.masked-value`  | UI             | [masked-value.md](template/INSPINIA/Components/UI/masked-value.md)   |    🟢    |     P1     |   🟢   |       ➕ valor sensível atrás de Mostrar/Ocultar com `aria-pressed` (irmão do field-display)       |
...
| 27a | `x-shared.reveal`         | Feedback  | —                                                                            |    🟢    |     P1     |   🟢   | ✅ Entrada fade-up padrão de blocos que trocam via `@if` (etapas, resultados); prop `delay` p/ stagger; `motion-reduce` sempre |

**Notas:**

- `toast` é a família `x-shared.toast-container` (com dois `<template>`: pílula/card) + motor JS único (`resources/js/admin/toast.js`). O `toast.blade.php` individual foi removido (era duplicado). Disparo no backend pelo trait `HT2ML\Core\Livewire\Concerns\EmiteNotificacoes`; posição/duração/estilo/máximo e a posição das confirmações são configuráveis em **Configurações → Notificações** (com preview ao vivo). Não depende de Alpine complexo.
...
| 28a | `x-shared.stat-tile`    | Data      | —                                                                    |    🟢    |     P1     |   🟢   | ✅ Tile de métrica (ícone soft + label + valor `tabular-nums` com count-up rAF); variants semânticas; props `countUp`/`delay` — resultado de importações e lotes |
...
|  #  | Blade destino           | Categoria | Doc                                                        | Vai usar | Prioridade | Status |                                          Decisão                                           |
| :-: | ----------------------- | --------- | ---------------------------------------------------------- | :------: | :--------: | :----: | :----------------------------------------------------------------------------------------: |
| 38a | `x-shared.masked-input` | Form      | [masked-input.md](template/INSPINIA/Forms/masked-input.md) |    🟢    |     P2     |   🟢   | ✅ **Base de todos os demais** — prop `mask`; um formato pontual não exige componente novo |
| 39  | `x-shared.cpf-input`    | Form      | [cpf-input.md](template/INSPINIA/Forms/cpf-input.md)       |    🟢    |     P2     |   🟢   |                       ➕ (wrapper de `masked-input` + DV no cliente)                       |
| 40  | `x-shared.cnpj-input`   | Form      | [cnpj-input.md](template/INSPINIA/Forms/cnpj-input.md)     |    🟢    |     P2     |   🟢   |                       ➕ (wrapper de `masked-input` + DV no cliente)                       |
| 41  | `x-shared.cep-input`    | Form      | [cep-input.md](template/INSPINIA/Forms/cep-input.md)       |    🟢    |     P2     |   🟢   |                             🧩 (Inputmask + ViaCEP helper JS)                              |
| 42  | `x-shared.phone-input`  | Form      | [phone-input.md](template/INSPINIA/Forms/phone-input.md)   |    🟢    |     P2     |   🟢   |                ➕ (wrapper de `masked-input`; máscara adaptativa fixo/cel)                 |
| 42a | `x-shared.pis-input`    | Form      | [pis-input.md](template/INSPINIA/Forms/pis-input.md)       |    🟢    |     P2     |   🟢   |                       ➕ (wrapper de `masked-input` + DV no cliente)                       |
| 42b | `x-shared.cid-input`    | Form      | [cid-input.md](template/INSPINIA/Forms/cid-input.md)       |    🟢    |     P2     |   🟢   |                ➕ (CID-10 `A99[.9]`, uppercase; o valor vai para o eSocial)                |
| 43  | `x-shared.money-input`  | Form      | [money-input.md](template/INSPINIA/Forms/money-input.md)   |    🟢    |     P2     |   🟢   |               🧩 (Alpine + `Intl.NumberFormat` + `HT2ML\Core\Support\Money\Money`)                |

### 8.4 Uploads, validação e wizard

|  #  | Blade destino                          | Categoria | Doc                                                      | Vai usar | Prioridade | Status |                                                                           Decisão                                                                            |
| :-: | -------------------------------------- | --------- | -------------------------------------------------------- | :------: | :--------: | :----: | :----------------------------------------------------------------------------------------------------------------------------------------------------------: |
| 44  | `x-shared.file-upload`                 | Form      | [file-upload.md](template/INSPINIA/Forms/file-upload.md) |    🟢    |     P2     |   🟢   |                      ✅ (modes `livewire`/`dropzone`; variants `card`/`compact`; dropzone com `size` `md`/`lg` hero + drag state rico)                       |
| 45  | — (mixin `@error` em `x-shared.input`) | Form      | [validation.md](template/INSPINIA/Forms/validation.md)   |    🟢    |     P2     |   🔴   |                                                                          ❌ (mixin)                                                                          |
| 46  | `x-shared.wizard`                      | Form      | [wizard.md](template/INSPINIA/Forms/wizard.md)           |    🟢    |     P2     |   🟢   |              ✅ (stepper server-driven; indicador puro, sem `wizard-step` — o conteúdo da etapa fica com o Livewire; conector e check animados)              |
| 46a | `x-shared.step-guide` (+`-item`)       | Form      | —                                                        |    🟢    |     P2     |   🟢   | ✅ Timeline vertical de instruções ("Como funciona") com ação contextual por passo (slot `action`); par de composição no padrão `accordion`/`accordion-item` |

**Notas de arquitetura:**

- **Scaffolding de CRUD (2026-07-12) — não copie, consuma.** As telas de CRUD tinham 93 arquivos duplicados (`_acoes`, `_lixeira-toggle`, `_ficha`, `_export-pdf`), gerados pelos stubs do gerador de CRUD. Hoje:
    - **Toolbar do grid** (Ver lixeira + Exportar PDF): view **única** `livewire.admin.partials.lixeira-toolbar`, entregue por `includeViewOnTop`. Detecta as duas metades pelas traits — `ComLixeira` (que exige `permissaoBase()`) e `ExportaPdf`. **38 arquivos eliminados.**
    - **Ações da linha** (kebab): view **única** `livewire.admin.partials.grid-acoes`, via o trait `ComAcoesCrud`, que fornece o `actionsFromView()`. O componente só declara `eventoVer()` e `rotaEditar()`. **22 arquivos eliminados.** CRUDs com ações extras (alternar status, personificar) mantêm o seu.
    - **Seção da ficha "Ver"**: `x-admin.ficha-section` (título + grade de `field-display`, com slot `footer` para o que não cabe na grade). A ficha em si continua por CRUD — cada uma mostra campos diferentes.
    - Os stubs `view-acoes`, `view-acoes-lixeira`, `view-lixeira-toggle` e `view-export-pdf` **foram removidos**: sem isso, o próximo `make:recurso` reintroduziria a duplicação.
- **Tabelas estáticas:** use `x-shared.static-table`. Ela traz `scope="col"` (sem ele o leitor de tela não associa a célula à coluna), empty state embutido e os tokens do tema. O `<table>` cru com classes `gray-*` **ignora o `data-theme`** — não acompanha o modo escuro nem os skins.
...
- **Máscaras (atualizado 2026-07-12):** `x-shared.masked-input` é a **base única** — recebe `mask` (string ou lista de alternativas) e repassa para `x-shared.input`. `cpf`/`cnpj`/`phone`/`pis`/`cid` são wrappers finos dela. Um formato pontual (ex.: uma placa numa tela só) usa a base direto, com `mask="..."` — **não** crie componente novo e **não** cole `data-af-inputmask` na view (§15.4 continua valendo: o atributo só existe dentro de `components/shared/`).
- **Dígito verificador no cliente:** `cpf`/`cnpj`/`pis-input` marcam `data-af-validate`, e o `validation-errors.js` faz um **pré-flight** no `salvar` — um documento com DV impossível é barrado sem gastar um round-trip. As Rules do PHP (`App\Rules\{Cpf,Cnpj,Pis}`) continuam sendo a autoridade; o fixture `tests/Fixtures/documentos-dv.json` é rodado contra as duas implementações, e divergência vira teste vermelho.
- `date-picker` e `date-range-picker` coexistem como componentes irmãos e compartilham a mesma base JS do Flatpickr. **Ambos são `wire:ignore` + Alpine/`$wire.entangle`**: o flatpickr muta o input (`type=hidden` + altInput) e o morph do Livewire desfazia a mutação, esvaziando o campo na tela a cada edição de qualquer outro campo. O campo visível tem **máscara de digitação** (o formato sai do `altFormat`) e o seletor de mês é `dropdown`.
- `money-input` **nunca** deve entregar float ao backend — sempre `int` centavos, via `HT2ML\Core\Support\Money\Money` (a doc antiga citava um `MoneyHelper::toCents()` que não existe).
- `cep-input` faz fetch ViaCEP no `@blur` e dispara `$dispatch('cep-filled', {logradouro, bairro, cidade, uf})`.
- ⚠️ **`@class()` não funciona dentro de uma tag de componente** (`<x-...>`): o parser do Blade não a reconhece como atributo e ecoa o componente inteiro como texto. Em tag HTML funciona; em componente, use `:class="$expressao"`.
- `wizard` é um stepper server-driven (implementado 2026-07-06 como indicador de progresso de fluxo — props `steps`/`current`, fallback compacto em telas pequenas); para navegação por seções o padrão segue sendo `tabs`.
...
| 49a | `x-rh::event-timeline`                       | Table     | (doc 06 §6.1)                                               |    🟢    |     P2     |   🟢   | 🧩 Timeline VERTICAL de eventos PONTUAIS (data + tipo + antes→depois) da linha do tempo do RH. Mora no **pacote extensao-rh** (namespace `rh::`, ADR-0015), não no core. Distinto do item 49 (períodos). Recebe itens já projetados pelo Livewire; sem `<select>` nativo, sem CSS custom. |
| 49b | `x-rh::afastamento-item`                     | Table     | (doc 06 §5)                                                 |    🟢    |     P2     |   🟢   |                 🧩 Linha de um afastamento (tipo + período + badge "em curso/atrasado" + CID já mascarado) na aba Afastamentos do RH. Ações vêm pelo `slot`. Pacote **extensao-rh** (`rh::`, ADR-0015). Recebe o array projetado pelo Livewire GerenciadorAfastamentosRh.                 |
...
As listagens de produção (`UsuariosTable`, `EmpresasTable`, `AuditoriaTable`) usam **PowerGrid** (`PowerGridComponent` + tema `HT2ML\Core\PowerGridThemes\InspiniaTheme`). Novo CRUD = nova classe `*Table` PowerGrid. As **ações por linha** são renderizadas via `actionsFromView()` apontando para uma view `_acoes.blade.php` que usa **`x-admin.row-actions`** (kebab). Não voltar a renderizar botões soltos por linha.
...
- Não existe `x-admin.timeline-item` no catálogo oficial. Para **períodos** (programações com início/fim/status) use `x-admin.timeline-table` (item 49); para **eventos pontuais** (histórico funcional do RH — data + tipo + antes→depois) há o `x-rh::event-timeline` (item 49a, no pacote extensao-rh).
...
| Data       | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-07 | **Infra do "Ver" (ficha de visualização):** `x-shared.field-display` (22a — promoção do `x-rh::field-display`, que agora delega) e `x-admin.ficha-drawer` (22b — composição `wide`+`blur` sobre o drawer, consumindo o trait `ComFicha` no Index). Item "Ver" no kebab gated por `can('view', $row)` (= `.listar`, sem permissão nova). Fluxo completo em `docs/visualizacao.md`; referência viva no módulo Exemplo; previews em `/admin/dev/components/{field-display,ficha-drawer}`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 2026-06-14 | **Sprint 0 — Primitivos cross-cutting (Batches 4–6):** `TabelaExport` (maatwebsite/excel) + logo BrandingService no PDF (`ExportarTabelaPdfAction` + `ExportavelDTO.logoUrl`); `HT2ML\Core\Livewire\Concerns\ExportaExcel` (trait par do ExportaPdf). Import padronizado: `BaseImport` (abstract + SkipsOnFailure), `ImportService`, `ImportLog` (model tenant-aware + Auditavel), `StatusImport` enum, `ImportResultadoDTO`. Anexos polimórficos: `Anexo` model + `Anexavel` trait + `GerenciadorAnexos` Livewire (`livewire:admin.shared.gerenciador-anexos`) — upload com `WithFileUploads`, remoção via `solicitarRemoverAnexo()` + bridge `confirm`/SweetAlert2 (`#[On('anexos::remover')]`). |
| 2026-07-07 | **Redesign das telas de importação (RH) — fundação de motion + 3 componentes novos.** Tokens de animação no `@theme` (`--animate-fade-up`, `--animate-pop-in`) e drag state rico do dropzone (`dz-drag-hover` acende com a cor da marca via token, em `plugins/_dropzone.css`). Novos: `x-shared.reveal` (27a — entrada fade-up com `delay` p/ stagger, substitui o padrão inline `x-init`+rAF), `x-shared.stat-tile` (28a — tile de métrica com count-up, extraído do partial de tiles do RH) e `x-shared.step-guide`/`step-guide-item` (46a — timeline vertical de instruções com ação contextual por passo). Upgrades aditivos: `wizard` (conector com `after:transition-colors` + check com pop) e `file-upload` (dropzone `size="lg"` hero; ícone da zona com `data-af-dz-zone-icon` + elevação no hover/drag; chip de preview com entrada animada). Testes: `Componentes/{Reveal,StatTile,StepGuide}Test` + casos novos no `FileUploadTest`. |
| 2026-07-07 | **`x-admin.drawer` — preset "workspace overlay" (`wide` + `blur` + `flush`, aditivos/opt-in).** `size="wide"` = painel quase tela cheia (`max-w-[min(120rem,94vw)]`; em top/bottom `max-h-[94vh]`); `blur` = `backdrop-blur-md` no overlay via `data-hs-overlay-options.backdropExtraClasses` do Preline (bg global de `_modal.css` preservado; ignorado sem backdrop); `flush` = corpo `p-0 overflow-hidden` para o slot gerenciar os próprios scrolls (rail + painel). Caso de referência: dicionário de colunas da importação de funcionários (RH), agora com rail de abas, busca cross-abas e tabela com coluna de exemplo + copy. Demais usos do drawer inalterados. Testes novos no `Componentes/DrawerTest`; preview atualizado em `/admin/dev/components/drawer`. |
| 2026-07-12 | **Follow-ups da revisão de frontend (PR #234): tabelas, filtros e toolbar dos grids.** (1) `x-shared.static-table` adotado nas **27 tabelas cruas** restantes (22 no RH: index-afastamentos/atestados/banco-horas/férias/HE, folha inteira, eSocial, inbox, espelho-ponto, consultas, meus-holerites…; 5 no core: acesso, auditoria, histórico de logins, filiais do form-empresa, permissões do form-usuario) — única exceção: `dicionario-importacao` (thead sticky ancorado no scroller do workspace, o wrapper do componente mataria o sticky). O componente ganhou colspan correto da linha de vazio no modo `x-slot:head` (conta os `<th>` do markup; caso novo no `StaticTableTest`). (2) **Filtros do PowerGrid colapsáveis**: `filter => 'outside'` no config — painel atrás do botão "Filtros" (override estilizado com contagem de ativos em `vendor/livewire-powergrid/.../header/filters.blade.php`), no lugar da linha permanente de inputs no thead; no painel cada controle tem `<label>` visível (a11y melhor que o sr-only do inline; `FiltroPowerGridA11yTest` atualizado). (3) **Toolbar dos grids em 1 linha**: override de `.../header.blade.php` põe o `includeViewOnTop` (lixeira + Exportar PDF) na MESMA linha flex de export/colunas/filtros/busca. (4) `wire:confirm` nativo **zerado** nas views: repeaters via trait `ConfirmaRemocaoRepeater` (mapa método→mensagem = allowlist do `#[On]`) + `solicitarReabrir`/`solicitarFechar`/`solicitarExcluir` pontuais (folha/espelho/batidas). (5) Listagem de funcionários: coluna **CPF** formatada (`Funcionario::cpfFormatado()`) + filtro, "Novo funcionário" no CTA. (6) Densidade do form de funcionário: **grid de 12 colunas** nas 6 seções (campos dimensionados pelo conteúdo), PCD em 3 colunas, `salario_tipo` e `estagio_natureza` (2 opções) viram `x-shared.radio-group` inline. |
