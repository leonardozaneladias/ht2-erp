...
| 43  | `x-shared.money-input`  | Form      | [money-input.md](template/INSPINIA/Forms/money-input.md)   |    🟢    |     P2     |   🟢   |               🧩 (Alpine + `Intl.NumberFormat` + `App\Support\Money\Money`)                |
...
- `money-input` **nunca** deve entregar float ao backend — sempre `int` centavos, via `App\Support\Money\Money` (a doc antiga citava um `MoneyHelper::toCents()` que não existe).
