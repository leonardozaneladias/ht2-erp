...
| **Sistema de notificações / avisos automáticos**                           | Não há motor de notificação (e-mail/push/in-app) na Fase 1: mover no organograma, atestado parado, documento a vencer **não** disparam aviso; os "alertas" são telas/consultas (KPIs/relatórios), não notificações enviadas | [13 — PEND-10](13-rastreabilidade-e-pendencias.md) |
...
| RNF-08 | **Lixeira**                            | `deleted_at` (SoftDeletes) + trait `HT2ML\Core\Livewire\Concerns\ComLixeira` com 3 níveis por módulo (`deletar`→lixeira, `restaurar`, `excluir_permanente`→force-delete); models implementam `HT2ML\Core\Models\Contracts\UsaSoftDeletes`                                                                                                                                           |
