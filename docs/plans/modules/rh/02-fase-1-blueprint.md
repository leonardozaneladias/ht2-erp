...
- **Lixeira** — models de negócio implementam `HT2ML\Core\Models\Contracts\UsaSoftDeletes` (`SoftDeletes`); as Tables usam `App\Livewire\Concerns\ComLixeira` (3 permissões por recurso: `deletar`→lixeira, `restaurar`, `excluir_permanente`→force-delete). Exceções append-only (`funcionario_funcao`, `escala_dias`, `escala_funcionario`, `funcionario_eventos`, `horas_extras`) **não** têm `deleted_at` — ver coluna "lixeira?" de cada bloco.
