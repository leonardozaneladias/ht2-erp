...
> Pacote `ht2erp/modulo-rh` (namespace `HT2ERP\Rh\`), aditivo ao core ([ADR-0015](../../../../architecture/adrs/ADR-0015-modulos-pacotes-composer.md)). Visão de produto em [00 §1.4](../00-prd.md); eixos satélites em [09 §8](../09-roadmap-fases.md). **Esta decisão é de produto/arquitetura de longo prazo — não amplia a Fase 1.**
...
- **Monólito**: um único `modulo-rh` que abarca tudo — incha, mistura domínios de cadência e equipe diferentes, e impede ativar/vender por partes conforme o cliente.
...
### Alt 1 — Monólito `modulo-rh` com tudo
...
- Contras: introduz **gestão de dependências entre pacotes** (versionar `modulo-rh` como dependência; estabilizar o contrato de API/eventos do núcleo). Aceitável — é o preço da modularidade.

## Decisão

**O RH é uma família de módulos-pacote: um núcleo + satélites.**

1. **Núcleo — `ht2erp/modulo-rh`.** Dono do agregado `funcionarios` ([ADR-RH-001](ADR-RH-001-funcionario-agregado-e-vinculo-adminuser.md)), do organograma/ACL ([ADR-RH-003](ADR-RH-003-acl-hierarquica-organograma.md)) e do histórico ([ADR-RH-005](ADR-RH-005-historico-eventos-imutaveis.md)). Entrega **Departamento Pessoal, jornada/HE, fundação de folha, folha, eSocial e ponto** nas fases de [09](../09-roadmap-fases.md). É a base que os satélites consomem.
2. **Satélites — pacotes Composer próprios** (vendor `ht2erp`, ex.: `ht2erp/modulo-sst`, `ht2erp/modulo-ats`), cobrindo os eixos vizinhos ([09 §8](../09-roadmap-fases.md)). Cada satélite:
    ...
    - declara **`ht2erp/modulo-rh` como dependência Composer** quando precisa do cadastro de pessoa; **referencia `funcionarios`/`admin_users` por FK nullable no pacote satélite** (não recria, não edita o núcleo);
...
- **Gestão de dependências entre pacotes**: os satélites versionam `modulo-rh`; o **contrato do núcleo** (API pública, eventos de domínio, FKs estáveis) precisa ser **estável e versionado** (SemVer) para não quebrar satélites a cada release.
- Mais **repositórios e CI** (cada pacote tem o seu — o `modulo-rh` já é repo próprio, ver a nota de distribuição/manutenção); custo operacional de orquestração.
