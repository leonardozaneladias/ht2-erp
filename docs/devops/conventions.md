...
- **Disposição padrão**: **declarada, não aplicada**. Cada seção do registro pode trazer `ordem` e um mapa `grupos` (`label`, `icone`, `ordem`), e cada item, `ordem` e `grupo`. Faixas: core 100–499, extensões e módulos do produto 500+. A extensão declara os seus por `ModuleRegistry::secaoDeMenu()` e `::grupoDeMenu()`. **O declarado é sugestão; o banco é a decisão de quem instalou** — a mesma semântica que `label` e `icone` já tinham.
  - Até 2026-08-25 isto era a `AplicarMenuPadraoAction`, que gravava 23 linhas em `menu_personalizacoes` no primeiro setup e hardcodava `grupo-tab-rh`, `ref-cnaes`, `ref-cfops`, `ref-ncms`, `rh-departamentos` e `rh-funcionarios` — o core conhecendo extensões pelo nome, sem uma linha de `use` que qualquer arch test de namespace pudesse ver. Ela e o `MenuPadraoSeeder` foram apagados; o guard A2 (`tests/Arch/CoreNaoConheceExtensaoTest.php`) impede a volta.
  - Tirar um item do grupo pela tela é uma **decisão**, não ausência dela: a `ReordenarItensMenuAction` grava `secao_key` quando o item tem grupo declarado, e é o que distingue "ninguém decidiu" de "decidiu-se que é fora de grupo". Sem isso o item voltaria ao grupo no render seguinte.
  - A chave do cache carrega uma impressão digital do registro: instalar uma extensão não passa por `invalidarCache()`, e sem ela a sidebar ficava até dez minutos sem os itens novos.
