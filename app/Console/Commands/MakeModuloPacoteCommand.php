 ...
 */
final class MakeModuloPacoteCommand extends Command
{
    protected $signature = 'make:modulo-pacote
        {nome : Nome do módulo no singular, PascalCase (ex.: Rh)}
        {--force : Sobrescreve a casca se o pacote já existir}';

    protected $description = 'Cria a casca de um módulo-pacote (HT2 ERP) em packages/ e registra o path repository.';
    ...
    public function handle(): int
    {
        ...
        if ($nome === '') {
            $this->error('Informe um nome de módulo válido (ex.: Rh).');

            return self::FAILURE;
        }

        $pkg = ModuloPacote::paraNome($nome);
        ...
        $stubDir = base_path('stubs/modulo-pacote');
        ...
        $repl = [
            ...
            '__VENDOR__' => (string) config('modulos.vendor'),
        ];
    ...
    private function gerarComposerJson(ModuloPacote $pkg): void
    {
        $ns = $pkg->namespaceBase . '\\';

        $composer = [
            'name' => $pkg->pacote,
            'description' => "Módulo {$pkg->studly} do HT2 ERP.",
            'type' => 'library',
            'license' => 'proprietary',
            'require' => [
                'php' => '^8.4',
    ...
    private function registrarPathRepository(): void
    {
        ...
        $url = (string) config('modulos.path') . '/*';
    ...
    private function resumo(ModuloPacote $pkg): void
    {
        $this->newLine();
