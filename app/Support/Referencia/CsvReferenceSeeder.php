 ...
 *
 * Vive em app/ e não em database/seeders/ porque `Database\\Seeders\\` é autoloadado
 * da aplicação: uma extensão instalada por Composer num Laravel limpo não
 * alcançaria essa raiz. Como base de extensão, precisa morar onde o pacote
 * consegue depender dela.
 *
...
abstract class CsvReferenceSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (! is_file($caminho)) {
            // Falha alto de propósito. Pular em silêncio faz o seed "passar"
            // com zero linha, e o sintoma só aparece muito depois — foi
            // exatamente o que aconteceu quando os CSVs fiscais foram para um
            // pacote e o caminho continuou apontando para o app.
            throw ImportacaoReferenciaException::arquivoAusente($this->tabela(), $caminho);
        }
    ...
    /**
     * Escreve em stdout do comando, se houver um anexado.
     *
     * Seeder::$command é propriedade TIPADA e só é preenchida por setCommand() —
     * rodando por db:seed programático ou em teste, ela fica não inicializada, e
     * `isset()` é a única forma de checar isso sem estourar Error. O PHPStan não
     * modela propriedade tipada não inicializada e acusa isset() como redundante.
     */
