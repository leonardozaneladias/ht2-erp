...
final class EstadoTable extends PowerGridComponent
{
    ...
    public function setUp(): array
    {
        return [
            PowerGrid::header()
                ->showSearchInput()
                ->showToggleColumns()
                ->includeViewOnTop('livewire.admin.referencia.estados._lixeira-toggle'),
            PowerGrid::footer()
                ->showPerPage()
                ->showRecordCount(),
    ...
    {
        if (! $row instanceof Estado) {
            return null;
        }

        return view('livewire.admin.referencia.estados._acoes', ['row' => $row, 'verLixeira' => $this->verLixeira]);
    }
