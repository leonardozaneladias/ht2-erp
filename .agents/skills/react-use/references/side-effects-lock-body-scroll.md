---
name: side-effects-lock-body-scroll
description: Lock scrolling of body element with useLockBodyScroll hook
---
...
- **`locked`**: `boolean` - Hook will lock scrolling on the body element if `true`, defaults to `true`
- **`elementRef`**: `RefObject<HTMLElement>` - The element ref object to find the body element. Can be either a ref to body or iframe element.

## Key Points

- Locks body scroll when enabled
- Useful for modals and overlays
- Supports iframe handling
- Can pass ref for performance
