<div class="space-y-6">
    ...
    <x-shared.tab-body>
        ...
        <x-shared.tab-panel id="aba-contato">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <x-shared.input type="email" name="email" label="Email" wire:model="email" required />
        ...
        <x-shared.tab-panel id="aba-comercial">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <x-shared.input type="number" name="quantidade" label="Quantidade" wire:model="quantidade" required />
                <x-shared.color-picker name="cor" label="Cor" wire:model="cor" clearable />
                <x-shared.select-search
                    name="categoria"
                    label="Categoria"
                    wire:model="categoria"
                    :options="['servico' => 'Servico', 'produto' => 'Produto', 'assinatura' => 'Assinatura']"
                    required
                />
                <x-shared.select-search
                    name="tags"
        ...
        <x-shared.tab-panel id="aba-datas-e-status">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <x-shared.date-picker name="data_inicio" label="Data inicio" wire:model="data_inicio" required />
                <x-shared.date-picker name="publicado_em" label="Publicado em" wire:model="publicado_em" />
                <x-shared.select-search
                    name="status"
                    label="Status"
                    wire:model="status"
                    :options="\App\Enums\StatusExemplo::options()"
                    required
                />
            </div>
        </x-shared.tab-panel>
