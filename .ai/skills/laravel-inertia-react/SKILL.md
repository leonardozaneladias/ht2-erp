                ...
                <div className="space-y-4">
                    {posts.data.map((post) => (
                        <article key={post.id} className="rounded-lg bg-white p-4 shadow">
                            ...
                            <p className="mt-2 text-sm text-gray-400">By {post.author.name}</p>
                        </article>
                    ))}
                </div>
            </div>
...
export default function Create({ categories }: Props) {
    ...
    return (
        <>
            ...
            <form onSubmit={handleSubmit} className="mx-auto max-w-2xl py-8">
                <div className="mb-4">
                ...
                <div className="flex gap-4">
                    ...
                    <Link href={route('posts.index')} className="rounded border px-4 py-2">
                        Cancel
                    </Link>
                </div>
            </form>
...
export default function FlashMessages() {
    ...
    return (
        <div className="fixed top-4 right-4 z-50">
            {flash.success && <div className="rounded bg-green-500 px-4 py-2 text-white shadow">{flash.success}</div>}
            {flash.error && <div className="rounded bg-red-500 px-4 py-2 text-white shadow">{flash.error}</div>}
        </div>
    );
}
```
