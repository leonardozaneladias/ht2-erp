...
export default function ExternalLink({ href, children, showIcon = true, className = '', ...props }: ExternalLinkProps) {
    return (
        <a
            href={href}
            target="_blank"
...
export default function DownloadLink({ href, filename, children, className = '' }: DownloadLinkProps) {
    return (
        <a href={href} download={filename} className={`inline-flex items-center gap-1 ${className}`}>
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
        ...
        <div className="space-y-8">
            ...
            <section>
                ...
                <div className="flex gap-4">
                    {socialLinks.map((link) => (
                        <ExternalLink key={link.id} href={link.url} className="text-blue-600 hover:text-blue-800">
                            {link.platform}
                        </ExternalLink>
                    ))}
                </div>
            ...
            <section>
                ...
                <p>
                    ...
                    or check out the{' '}
                    <ExternalLink href="https://github.com/company/repo" className="text-blue-600">
                        GitHub repository
                    </ExternalLink>
                    .
                </p>
