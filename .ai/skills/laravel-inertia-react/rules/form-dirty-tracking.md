...
useEffect(() => {
    setIsDirty(data.name !== originalData.name || data.email !== originalData.email);
}, [data]);
```
        ...
        <div>
            ...
            {isDirty && (
                <div className="mb-4 flex items-center justify-between rounded-md bg-amber-50 px-4 py-2 text-amber-800">
                    ...
                    <button type="button" onClick={() => reset()} className="text-sm underline hover:no-underline">
                        Discard changes
                    </button>
                </div>
            )}
            ...
            <form onSubmit={submit} className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-sm text-gray-500">{isDirty ? 'Unsaved changes' : 'All changes saved'}</span>
                </div>
            </form>
        </div>
    );
