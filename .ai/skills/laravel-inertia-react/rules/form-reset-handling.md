    ...
    return (
        <form onSubmit={submit}>
            ...
            <div className="flex items-center gap-4">
                ...
                {isDirty && (
                    <button type="button" onClick={handleRevert} className="text-gray-600 hover:text-gray-800">
                        Discard Changes
                    </button>
                )}
            </div>

            {isDirty && <p className="mt-2 text-sm text-amber-600">You have unsaved changes</p>}
        </form>
    );
}
```
